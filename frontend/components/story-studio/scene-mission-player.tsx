...
function Frame({ spec, badge, children }: { spec: TypedSceneSpec; badge: string; children: ReactNode }) {
  return (
    <article className={`rounded-[2rem] border-4 p-4 sm:p-5 ${PALETTE_CLASS_MAP[spec.paletteKey]}`}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="max-w-2xl">
          ...
          <p className="mt-2 text-sm font-semibold leading-relaxed text-slate-700">{spec.instruction}</p>
        </div>
...
function StateSliderMission({
  ...
  const currentStage = [...spec.stages].reverse().find((stage) => value >= stage.threshold) ?? spec.stages[0];
    ...
    <Frame spec={spec} badge="Agentic Scene Spec">
      <div className="mt-4 grid gap-4 lg:grid-cols-[1fr_1.15fr]">
        <div className="rounded-3xl border-2 border-white/80 bg-white p-4">
          ...
          <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-center text-7xl">{currentStage.emoji}</div>
