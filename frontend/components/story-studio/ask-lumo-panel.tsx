...
import { getLearningCoachReply } from "@/lib/story-studio/generate-learning-coach";
...
function buildCoachBubbleText(params: {
  ...
  return [
    ...
    `Think: ${params.reflectionQuestion}`,
  ]
    .filter(Boolean)
    .join(" ");
}
...
function explainWord(word: string) {
  ...
  const dictionary: Record<string, string> = {
    ...
    biome: "Biome means a big kind of place, like a desert or snowy land.",
...
export function AskLumoPanel({ lesson, hintText }: AskLumoPanelProps) {
  const welcomeMessage: Message = {
    id: "welcome",
    role: "assistant",
    text: "Hi. I am Lumo. Tap a help chip for quick help, or ask Coach Mode when you are stuck.",
  };
  ...
  return (
    <section className="relative overflow-hidden rounded-[2rem] border border-white/80 bg-[radial-gradient(circle_at_10%_10%,#fdf2f8,transparent_28%),radial-gradient(circle_at_95%_90%,#dbeafe,transparent_34%),linear-gradient(150deg,#fffaf5,#ffffff_48%,#f5f3ff)] p-4 shadow-[0_24px_55px_-34px_rgba(192,38,211,0.42)]">
      <div className="flex items-start gap-3">
        <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-[1.4rem] border border-fuchsia-200 bg-gradient-to-br from-fuchsia-100 to-cyan-100 text-3xl">
          ✨
        </div>
        <div>
          <p className="text-xs font-black uppercase tracking-[0.22em] text-fuchsia-700">Ask Lumo</p>
      ...
      <div className="mt-4 rounded-[1.5rem] border border-cyan-200 bg-cyan-50/80 p-3">
        ...
        <div className="mt-2 flex gap-2">
          <input
            ...
            className="flex-1 rounded-2xl border border-cyan-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 outline-none transition focus:border-cyan-400"
          />
          <button
            type="button"
        ...
        {coachError ? <p className="mt-2 text-xs font-semibold text-rose-700">{coachError}</p> : null}
      </div>
      ...
      <div className="mt-4 rounded-[1.5rem] border border-slate-200 bg-white/90 p-3">
        ...
        <div className="mt-2 flex gap-2">
          <input
            ...
            className="flex-1 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-semibold text-slate-700 outline-none transition focus:border-fuchsia-300"
          />
          <button
            type="button"
      ...
      <button
        ...
        className="mt-3 w-full rounded-2xl border border-cyan-200 bg-gradient-to-r from-cyan-50 to-sky-100 px-4 py-3 text-sm font-black uppercase tracking-wide text-cyan-900"
      >
