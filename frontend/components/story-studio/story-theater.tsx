...
import { generateStoryNarration } from "@/lib/story-studio/generate-story-narration";
import { generateStoryExperience } from "@/lib/story-studio/generate-story-experience";
import type { LessonSpec } from "@/lib/story-studio/lesson-spec";
import { readStoryExperience, saveStoryExperience } from "@/lib/story-studio/session-state";
import type { StoryExperienceResponse } from "@/lib/story-studio/story-experience";
...
export function StoryTheater({ lesson, storyCompleted, onUnlockMission }: StoryTheaterProps) {
  ...
  async function handleBeginStory() {
    ...
    try {
      ...
      setStatus("error");
      setErrorMessage(
        error instanceof Error ? error.message : "Story mode could not be generated right now."
      );
    }
  }
  ...
  async function handlePlayNarration() {
    ...
    if (
      ...
      try {
        ...
        // Fall through to browser speech if on-demand narration generation fails.
      } finally {
    ...
    return (
      <section className="rounded-[2rem] border-4 border-emerald-200 bg-[radial-gradient(circle_at_10%_8%,#bbf7d0,transparent_30%),radial-gradient(circle_at_95%_85%,#bfdbfe,transparent_28%),linear-gradient(140deg,#ecfdf5,#f0fdfa)] p-5 shadow-[0_20px_50px_-28px_rgba(16,185,129,0.5)]">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <button
            type="button"
            onClick={() => void handlePlayNarration()}
    ...
    <section className="relative overflow-hidden rounded-[2rem] border-4 border-fuchsia-200 bg-[radial-gradient(circle_at_8%_8%,#fae8ff,transparent_28%),radial-gradient(circle_at_92%_12%,#bfdbfe,transparent_30%),radial-gradient(circle_at_90%_90%,#fde68a,transparent_25%),linear-gradient(145deg,#fff7ed,#ffffff_48%,#eef2ff)] p-5 shadow-[0_30px_80px_-40px_rgba(168,85,247,0.62)] sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="max-w-2xl">
          <p className="text-xs font-black uppercase tracking-[0.25em] text-fuchsia-700">Story Mode</p>
      ...
      {status === "idle" ? (
        <div className="mt-5 grid gap-4 xl:grid-cols-[1.3fr_0.9fr]">
          <div className="rounded-[1.75rem] border-2 border-white/80 bg-white/80 p-5 shadow-sm">
            <p className="text-xs font-black uppercase tracking-[0.2em] text-cyan-700">Adventure Brief</p>
            <h3 className="mt-2 text-2xl font-black text-slate-900">Start with a cinematic learning moment</h3>
            <p className="mt-3 text-sm font-medium text-slate-700">
              The story uses Gemini-generated story panels and optional spoken narration to
              introduce <strong>{lesson.unitOrModule}</strong> before the hands-on activity begins.
            </p>
            <div className="mt-5 flex flex-wrap gap-3">
              <button
                type="button"
                onClick={() => void handleBeginStory()}
          ...
          <div className="grid gap-3">
            {["Meet the learner", "See the world", "Hear the guide"].map((label, index) => (
              <div
                key={label}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-medium text-slate-600">
                      Designed to feel like a kid-facing adventure, not a plain worksheet.
                    </p>
                  </div>
                </div>
              </div>
      ...
      {status === "loading" ? (
        <div className="mt-5 grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
          <div className="rounded-[1.75rem] border-2 border-white/80 bg-white/80 p-5 shadow-sm">
            <p className="text-xs font-black uppercase tracking-[0.2em] text-cyan-700">Story Engine</p>
            <h3 className="mt-2 text-2xl font-black text-slate-900">Painting the adventure in real time</h3>
      ...
      {status === "error" ? (
        <div className="mt-5 rounded-[1.75rem] border-2 border-rose-200 bg-rose-50 p-5">
          <p className="text-xs font-black uppercase tracking-[0.2em] text-rose-700">Story Mode Error</p>
          ...
          <div className="rounded-[1.75rem] border-2 border-white/80 bg-white/85 p-5 shadow-sm">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div className="max-w-2xl">
                ...
                <h3 className="mt-2 text-2xl font-black text-slate-900">{experience.story.title}</h3>
                <p className="mt-2 text-sm font-medium text-slate-700">{experience.story.introLine}</p>
              </div>
              <div className="flex flex-wrap gap-2">
                <button
                  type="button"
                  onClick={isNarrating ? handleStopNarration : () => void handlePlayNarration()}
