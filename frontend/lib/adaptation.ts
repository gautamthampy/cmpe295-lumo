...
export function deriveAdaptation(
  ...
  const shouldAdapt =
    progress.consecutiveIncorrect >= 2 || progress.hintRequests >= 2;
  ...
  const nextMechanicId = getAlternateMechanic(
    currentMechanicId,
    lesson.conceptFamily
  );
  const hintText =
    lesson.hintLadder[Math.min(progress.consecutiveIncorrect, lesson.hintLadder.length - 1)]
      ?.text || "Let's try one easier example together.";
